...
class _EasyStarsState extends State<EasyStars> with TickerProviderStateMixin {
    ...
    switch (widget.config.arrangement) {
      ...
      case StarArrangement.spiral:
        double angle = (2 * math.pi * widget.config.spiralTurns * index) /
            widget.config.starCount;
        double radius =
            widget.config.arrangementRadius * index / widget.config.starCount;
  ...
  void _animateRatingChange() {
    for (int i = 0; i < _starControllers.length; i++) {
      Future.delayed(
        Duration(
            milliseconds:
                i * widget.animationConfig.staggerDelay.inMilliseconds),
        () {
          if (mounted) {
  ...
  void _updateRating(double newRating) {
    if (newRating != _currentRating) {
      setState(() {
        _currentRating =
            newRating.clamp(widget.config.minRating, widget.config.maxRating);
      });
  ...
  Widget _buildAnimatedStar(int index, bool isFilled, bool isHalf) {
    return AnimatedBuilder(
      animation:
          Listenable.merge([_starAnimations[index], _hoverAnimations[index]]),
  ...
  Matrix4 _getTransformMatrix(int index) {
    ...
    switch (widget.animationConfig.animationType) {
      ...
      case StarAnimation.pulse:
        double pulse = 1.0 +
            (widget.animationConfig.pulseScale - 1.0) *
  ...
  Widget _buildStar(int index, bool isFilled, bool isHalf) {
    ...
    if (isHovered && widget.animationConfig.animateOnHover) {
      ...
      if (widget.config.customIcons != null &&
          index < widget.config.customIcons!.length) {
    ...
    if (widget.config.gradientColors != null &&
        widget.config.gradientColors!.length > 1) {
      star = ShaderMask(
  ...
  double _calculateRatingFromPosition(
      double x, double y, BoxConstraints constraints) {
    ...
    if (widget.config.direction == StarDirection.horizontal) {
      ...
      return math.max(
          0.0, math.min(widget.config.starCount.toDouble(), position));
    } else {
  ...
  double _calculateRatingFromDrag(
      DragUpdateDetails details, BoxConstraints constraints) {
    switch (widget.config.arrangement) {
      case StarArrangement.linear:
        return _calculateRatingFromPosition(
            details.localPosition.dx, details.localPosition.dy, constraints);

      case StarArrangement.circular:
      case StarArrangement.arc:
        double centerX = constraints.maxWidth / 2;
        double centerY = constraints.maxHeight / 2;
        double angle = math.atan2(details.localPosition.dy - centerY,
  ...
  Widget _buildStarArrangement() {
    ...
    double containerSize =
        (widget.config.arrangementRadius * 2) + (maxSize * 2);
    ...
    return SizedBox(
      ...
      child: Stack(
        children: List.generate(widget.config.starCount, (i) {
          ...
          Offset position =
              _calculateStarPosition(i, Size(containerSize, containerSize));
          ...
          ? Row(
              children: [
                ...
                Text(
                  ...
                  style: widget.config.ratingTextStyle ??
